...
<x-layout>
    <x-card class="p-10 max-w-lg mx-auto mt-24">
        <header class="text-center">
            ...
            <p class="mb-4">Make a new question</p>
        </header>
            ...
            <div class="mb-6">
                ...
                <select name="q_type_id" id="q_type_id" class="border border-gray-200 rounded p-2 w-full">
                    @foreach ($types as $type)
                        <option value="{{$type->id}}" {{old('q_type_id') == $type->id? 'selected' : ''}}> {{$type->name}} </option>
                    @endforeach
                </select>
            </div>
            <div class="mb-6">
                ...
                <select name="q_category_id" id="q_category_id" class="border border-gray-200 rounded p-2 w-full">
                    @foreach ($category as $cat)
                        <option value="{{$cat->id}}" {{old('q_category_id') == $cat->id? 'selected' : ''}}> {{$cat->name}} </option>
                    @endforeach
                </select>
            </div>
            <div class="mb-6">
                ...
                <input
                    ...
                    name="sentence" value="{{old('sentence')}}"
                />
